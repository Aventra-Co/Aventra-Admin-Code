...
export default function FoodCategoryTableCoponenet({ thead, tbody }) {
    ...
    const filteredUsers = categoryDetails.filter((user) => {
        ...
        return (
            ...
            user.food_category_arabic?.toLowerCase().includes(lowercasedTerm) ||
            user.food_category_french?.toLowerCase().includes(lowercasedTerm) ||
            user.food_category_italian?.toLowerCase().includes(lowercasedTerm) ||
            user.food_category_korean?.toLowerCase().includes(lowercasedTerm) ||
    ...
    const fetchCategory = () => {
        ...
        axios.get(API_URL + "/fetch_food_category")
            .then((response) => {
                setcategoryDetails(response.data.food_category_arr || []);
                setLoading(false)
            })
            .catch((error) => {
    ...
    const handleUserAction = (action, item) => {
        if (action === 'delete') {
            setAlertModal(true);
            setDeleteId(item.food_category_id);

        }
        if(action === 'edit'){
            // <Link to={APP_PREFIX_PATH + `/edit-food-category/${encode(item.food_category_id)}`}>
            //                                     <ButtonComponent title="Edit" className="material-icons edit">edit</ButtonComponent></Link>
            navigate(APP_PREFIX_PATH + `/edit-food-category/${encode(item.food_category_id)}`);
    ...
    const DeleteDestination = () => {
        if (DeleteId) {
            axios.post(API_URL + '/delete_food_category', { food_category_id: DeleteId }).then((res) => {
                if (res.data.success) {
        ...
        <>
            ...
            <Row xs={1} sm={2} xl={4} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Col style={{ textAlign: 'right', marginBottom: '5px' }}>
                    <Link to={APP_PREFIX_PATH + '/add-food-category'}>
                        <button style={{ background: '#2b77e5', padding: '7px 13px', color: '#fff', borderRadius: '5px' }}> <AddIcon className="me-2" /> {t("Add Category")} </button></Link>
                </Col>
                ...
                <div className="mc-table-responsive">
                    <table className="mc-table">
                        <thead className="mc-table-head primary">
                            <tr>
                                ...
                                <th>{t("actions")}</th>
                                <th>{t("Food Category English")}</th>
                                <th>{t("Food Category Arabic")}</th>
                                <th>{t("Food Category French")}</th>
                                <th>{t("Food Category Italian")}</th>
                                <th>{t("Food Category Korean")}</th>
                        ...
                        <tbody className="mc-table-body even">
                            {currentUsers?.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{item.food_category_arabic || 'NA'}</td>
                                    <td>{item.food_category_french || 'NA'}</td>
                                    <td>{item.food_category_italian || 'NA'}</td>
                                    <td>{item.food_category_korean || 'NA'}</td>
                    ...
                    <Modal show={successModel} onHide={() => setsuccessModel(false)}>
                        <div className="mc-alert-modal">
                            ...
                            <h3>{t('success')}</h3><br />
                            <p>Food Category deleted successfully.</p>
                            <Modal.Footer>
                    ...
                    <Modal show={alertModal} onHide={() => setAlertModal(false)}>
                        <div className="mc-alert-modal">
                            ...
                            <h3>are your sure!</h3>
                            <p>Want to delete this food category?</p>
                            <Modal.Footer>
