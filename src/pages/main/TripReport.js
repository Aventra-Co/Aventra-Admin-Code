...
export default function TripReport() {
  ...
  const fetchtripDetails = () => {
    ...
    axios
      .post(API_URL + '/trip_tabular_report', {
        from_date: fromDate,
        to_date: toDate
      })
      .then(response => {
    ...
    const ws = XLSX.utils.json_to_sheet(
      tripDetails.map((user, index) => {
        ...
        return {
          ...
          'Price (KWD/Hr)': user.price_per_hour || 'NA',
          'Advertisement Type':
  ...
  return (
    <PageLayout>
      ...
      <Helmet>
        <title>Aventra | Trip-Report</title>
      </Helmet>

      <Col xl={12}>
        <div className='mc-card'>
          <div className='mc-breadcrumb'>
            <h3 className='mc-breadcrumb-title'>{t('Trip Report')}</h3>
      ...
      <div className='mc-card p-lg-4'>
        <Col xl={12}>
          <Card.Header>
            <Card.Title as='h5'>{t('Trip Report')}</Card.Title>
          </Card.Header>
          <br />
          <Card.Body>
