...
export default function StaffTabularReport() {
  ...
  const fetchUserDetails = () => {
    ...
    axios
      .post(API_URL + '/staff_tabular_report', {
        from_date: fromDate,
        to_date: toDate
      })
      .then(response => {
  ...
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(
      userDetails.map((user, index) => {
        ...
        return {
          ...
          // Mobile: (user.mobile),
          'Create Date & Time':
            moment(createDateTime).format('DD-MM-YYYY HH:mm A')
        }
      })
    )
    ...
    saveAs(blob, 'StaffReport.xlsx')
  }

  return (
    <PageLayout>
      <Helmet>
        <title>Aventra | Staff-Report</title>
      </Helmet>
      <Col xl={12}>
        <div className='mc-card'>
          <div className='mc-breadcrumb'>
            <h3 className='mc-breadcrumb-title'>{t('Staff report')}</h3>
            <ul className='mc-breadcrumb-list'>
              ...
              <li className='mc-breadcrumb-item'>{t('Staff_list')}</li>
            </ul>
          </div>
        </div>
      </Col>
      <div className='mc-card p-lg-4'>
        <Col xl={12}>
          <Card.Header>
            <Card.Title as='h5'>{t('Staff report')}</Card.Title>
          </Card.Header>
          <br />
          <Card.Body>
            <Row>
      ...
      <div className='mc-card p-lg-4'>
        <Col xl={12}>
          ...
          {/* <Card.Body>
                        <Table responsive hover className="mc-table">
          ...
          <div className='mc-table-responsive'>
            <table className='mc-table'>
              ...
              <tbody className='mc-table-body even'>
                {userDetails.length > 0 && userDetails ? (
                  <>
                    {' '}
                    {userDetails?.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          ...
                          >
                            {item.active_flag == 0 ? 'Deactive' : 'Active'}
                          </p>
                        </td>
